...
labelPCSampleByGmv <- function(df) {
	...
	C <- sum(df$item_gmv_sum_1day) / sum(df$item_book_pv_1day)

	row <- order(df$item_book_pv_1day,decreasing=TRUE)
	M <- df$item_book_pv_1day[row[100]]
...
labelAPPSampleByGmv <- function(df) {
	...
	M <- df$item_book_pv_1day_app[row[100]]
...
labelSample <- function(args) {
    ...
    pcGmvFilename <- paste(args[1],"reg.label",sep=".")
