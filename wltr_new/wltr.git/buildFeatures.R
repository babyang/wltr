...
expOnlineSmooth <- function(df) {
	...
	alpha <- ifelse(t<=0.7,1,1.0 / ( 1.0 + exp(t - 1) ))
    df$pcBookPV <- alpha * df$pcBookPV  + exp(1)*df$item_book_pv_0day
   df$pcDetailPV <- alpha * df$pcDetailPV  + exp(1)*df$item_click_pv_0day
    df$pcGmvSum <- alpha * df$pcGmvSum  + 0.01*exp(1)*df$item_gmv_sum_0day
    df$pcGmvCount <- alpha * df$pcGmvCount  + exp(1)*df$item_gmv_sum_0day / df$price
    
    df$appBookPV <- alpha * df$appBookPV  + exp(1)*df$item_book_pv_0day_app
    df$appDetailPV <- alpha * df$appDetailPV  + exp(1)*df$item_click_pv_0day_app
    df$appGmvSum <- alpha * df$appGmvSum  + 0.01*exp(1)*df$item_gmv_sum_0day_app
    df$appGmvCount <- alpha * df$appGmvCount  + exp(1)*df$item_gmv_sum_0day_app / df$price
    return(df)
}
...
#            "pcSpGmvSum","pcSpDetailPV","pcSpPGmvDetail")
   cols <- c("pcDetailPV","pcGmvSum","pcGmvCount","pcPGmvDetail")
             
# cols <- c(cols,"appDetailPV","appGmvSum","appGmvCount","appSpGmvSum",
#             "appSpDetailPV","appPGmvDetail","appSpPGmvDetail")
